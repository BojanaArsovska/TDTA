...
# COUNT(*) AS COMMIT_COUNT third column, counts the
    ...
    cursor.execute("SELECT author, file_name, SUM(ABS(changes)) AS SCORE, COUNT(author) AS COUNT FROM commits GROUP BY file_name ORDER BY SUM(changes) DESC")
    result1_2 = cursor.fetchall()
    # Convert result1 to dataframe for better view
    df1_1 = pd.DataFrame(result1_1, columns=['Author', 'File', 'Number of Edits by <Author>', 'Number of Commits by <Author> including<File>'])
    df1_2 = pd.DataFrame(result1_2, columns=['Author', 'File Name', 'Code Churn - Total number of lines added/deleted/edited', 'Author Edits/Commits with the file '])
